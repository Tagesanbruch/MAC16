...
// Stage 3: CSA Layer 1 (8->6) + CSA Layer 2 (6->4) + feedback rows
// Stage 4: CSA-only compression (6->2)
//============================================================================
module mult16_booth_dcs (
    ...
    //=========================================================================    
    // Stage 3: CSA Layer 1 (8->6) + CSA Layer 2 (6->4)
    //=========================================================================
    ...
    wire [39:0] csa8_sum, csa8_carry;
    csa #(.WIDTH(40)) u_csa8 (
        .a(csa7_sum),
        .b({csa7_carry[38:0], 1'b0}),
        .c({csa6_carry[38:0], 1'b0}),
        .sum(csa8_sum),
        .carry(csa8_carry)
    );
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            result_sum   <= csa8_sum;
            result_carry <= {csa8_carry[38:0], 1'b0};
            valid_out <= 1'b1;
        end else begin
            valid_out <= 1'b0;
        end
