`timescale 1ns/1ps
//============================================================================
// Experiment O: 7-Stage Super-Deep Pipeline Booth Multiplier
// 
// Key Innovation: Even deeper pipeline than Exp N with additional stage splits
// - Stage 1: Booth Encoding
// - Stage 2: PPG (Partial Product Generation)
// - Stage 3: CSA Layer 1 (8→6 rows)
// - Stage 4: CSA Layer 2 (6→4 rows) + Feedback injection
// - Stage 5: 4:2 Compressor (6→2 rows with feedback)
// - Stage 6: Final CSA for accumulator merge
// - Stage 7: Output (sum, carry) - NO CPA in loop
//
// Target: Reduce logic depth per stage to ~8 FO4 for SS corner timing
//============================================================================
module mult16_booth_7stage (
    input  wire         clk,
    input  wire         rst_n,
    input  wire  [15:0] a,
    input  wire  [15:0] b,
    ...
    //=========================================================================
    // Stage 2: PPG (Partial Product Generation) - ISOLATED STAGE
    //=========================================================================
    ...
    // Pipeline register - Stage 2 (PPG only, no CSA logic)
    ...
    //=========================================================================
    // Stage 4: CSA Layer 2 (6→4 rows) - Inject feedback here
    //=========================================================================
    ...
    //=========================================================================
    // Stage 5: 4:2 Compressor + Feedback injection (6→4 rows)
    //=========================================================================
    ...
    // Now merge with feedback: comp_sum, comp_carry, acc_sum, acc_carry = 4 rows
    ...
    //=========================================================================
    // Stage 6: Final 4:2 Compressor with feedback (4→2 rows)
    //=========================================================================
    ...
    // Pipeline register - Stage 6 (Output in carry-save form!)
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            result_sum   <= final_sum_s6;
            result_carry <= final_carry_s6;
            valid_out <= 1'b1;
        end else begin
            valid_out <= 1'b0;
        end
