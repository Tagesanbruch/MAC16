...
module mult16_booth_dcs (
    ...
    //=========================================================================
    // Stage 1: Booth Encoding with Operand Gating (Power Optimization)
    //=========================================================================
    ...
    // Operand gating: gate inputs to 0 when not valid to reduce switching
    wire [15:0] a_gated = valid_in ? a : 16'b0;
    wire [15:0] b_gated = valid_in ? b : 16'b0;
    
    wire [7:0]  neg_w, zero_w, two_w;
    booth_encoder u_booth (
        .b(b_gated),  // Use gated operand
        .neg(neg_w),
        .zero(zero_w),
        .two(two_w)
    );
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            zero_s1 <= zero_w;
            two_s1  <= two_w;
            a_s1    <= a_gated;  // Use gated operand
