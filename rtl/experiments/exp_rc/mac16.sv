`timescale 1ns/1ps
//============================================================================
// Experiment RC: Split VMA + Non-uniform Retiming
// 
// Key Optimization: Split 40-bit VMA into pipelined low/high halves
// - Uses extra latency budget (4→5 cycles) to split VMA
// - Reduces VMA critical path by ~50%
//
// Architecture:
// - Stage 1: Booth Encoding
// - Stage 2: PPG  
// - Stage 3: CSA Layers + Feedback
// - Stage 4: 4:2 Compressor + CSA → (Sum, Carry) output
// - Stage 5: VMA Stage 1 (Low 20-bit add + generate carries for high)
// - Stage 6: VMA Stage 2 (High 20-bit add with carry-in) → Final result
//
// This uses 5-cycle latency (matches constraint exactly)
//============================================================================
module mac16 (
    input  logic        clk,
    input  logic        rst_n,
    ...
    //=========================================================================
    // Split VMA: 2-stage pipelined adder
    //=========================================================================
    
    // VMA Stage 1: Low 20-bit addition + Carry generation
    logic [ACC_WIDTH-1:0] vma_sum_s1, vma_carry_s1;
    logic vma_valid_s1;
    logic [19:0] vma_low_result;
    logic vma_low_cout;
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            vma_sum_s1 <= mult_result_sum;
            vma_carry_s1 <= mult_result_carry;
            vma_valid_s1 <= 1'b1;
        end else begin
            vma_valid_s1 <= 1'b0;
        end
    ...
    // VMA Stage 2: High 20-bit addition with carry-in
    wire [19:0] high_a = vma_sum_s1[39:20];
    wire [19:0] high_b = vma_carry_s1[39:20];
    wire [19:0] vma_high_result;
    wire vma_high_cout;
    
    // Recompute low bits from registered values
    wire [19:0] low_a_s1 = vma_sum_s1[19:0];
    wire [19:0] low_b_s1 = vma_carry_s1[19:0];
    wire [20:0] low_sum_s1 = low_a_s1 + low_b_s1;
    
    assign {vma_high_cout, vma_high_result} = high_a + high_b + low_sum_s1[20];
    
    wire [ACC_WIDTH-1:0] vma_result = {vma_high_result, low_sum_s1[19:0]};
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                S_MULT_WAIT: begin
                    // Wait for split VMA pipeline (vma_valid_s1 is one cycle after mult_valid_out)
                    if (vma_valid_s1) begin
