`timescale 1ns/1ps
//============================================================================
// Experiment TB: Triple-Vector DCS Accumulator
//
// Keeps 3 carry-save vectors in feedback path to reduce compression depth.
//============================================================================
module mac16 (
    input  logic        clk,
    input  logic        rst_n,
    ...
    logic [ACC_WIDTH-1:0] acc_extra;

    logic [ACC_WIDTH-1:0] prev_sum;
    logic [ACC_WIDTH-1:0] prev_carry;
    logic [ACC_WIDTH-1:0] prev_extra;
    ...
    logic [ACC_WIDTH-1:0] mult_result_sum, mult_result_carry, mult_result_extra;
    logic mult_valid_out;

    logic [ACC_WIDTH-1:0] feedback_sum, feedback_carry, feedback_extra;
    ...
    always_comb begin
        if (first_op) begin
            ...
            feedback_extra = prev_extra;
        end else begin
            feedback_sum   = acc_sum;
            feedback_carry = acc_carry;
            feedback_extra = acc_extra;
        end
    end
    ...
    mult16_booth_dcs u_mult (
        ...
        .acc_extra_in(feedback_extra),
        .result_sum(mult_result_sum),
        .result_carry(mult_result_carry),
        .result_extra(mult_result_extra),
        .valid_out(mult_valid_out)
    );

    wire [ACC_WIDTH:0] vma0 = {1'b0, mult_result_sum} + {1'b0, mult_result_carry};
    wire [ACC_WIDTH:0] vma1 = vma0 + {1'b0, mult_result_extra};
    wire [OUTPUT_BITS-1:0] mac_result = vma1[OUTPUT_BITS-1:0];
    wire vma_overflow = vma1[OUTPUT_BITS];
                ...
                S_MULT_WAIT: begin
                    if (mult_valid_out) begin
                        if (mode_r == 1'b0) begin
                            ...
                            prev_extra <= mult_result_extra;
                        end else begin
                            acc_sum   <= mult_result_sum;
                            acc_carry <= mult_result_carry;
                            acc_extra <= mult_result_extra;
                        end
