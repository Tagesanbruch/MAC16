`timescale 1ns/1ps
//============================================================================
// Experiment TB: Triple-Vector DCS Booth Multiplier
//
// Stage 4 keeps 3 vectors instead of compressing to 2.
//============================================================================
module mult16_booth_dcs (
    ...
    input  wire  [39:0] acc_extra_in,
    output reg   [39:0] result_sum,
    output reg   [39:0] result_carry,
    output reg   [39:0] result_extra,
    ...
    reg        valid_s1;
    reg [15:0] a_s1;
    reg [39:0] acc_sum_s1, acc_carry_s1, acc_extra_s1;
    ...
    reg [39:0] acc_sum_s2, acc_carry_s2, acc_extra_s2;
    reg        valid_s2;
    ...
    reg [39:0] row0_s3, row1_s3, row2_s3, row3_s3, row4_s3, row5_s3, row6_s3;
    reg        valid_s3;

    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            row0_s3 <= 40'b0; row1_s3 <= 40'b0; row2_s3 <= 40'b0;
            row3_s3 <= 40'b0; row4_s3 <= 40'b0; row5_s3 <= 40'b0; row6_s3 <= 40'b0;
            valid_s3 <= 1'b0;
        end else if (valid_s2) begin
            row0_s3 <= acc_sum_s2;
            row1_s3 <= acc_carry_s2;
            row2_s3 <= acc_extra_s2;
            row3_s3 <= csa3_sum;
            row4_s3 <= {csa3_carry[38:0], 1'b0};
            row5_s3 <= csa4_sum;
            row6_s3 <= {csa4_carry[38:0], 1'b0};
            valid_s3 <= 1'b1;
        end else begin
            valid_s3 <= 1'b0;
        end
    ...
    // Stage 4: 7 rows -> 3 rows (triple-vector)
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            result_sum   <= csa7_sum;
            result_carry <= {csa7_carry[38:0], 1'b0};
            result_extra <= {csa6_carry[38:0], 1'b0};
            valid_out <= 1'b1;
        end else begin
            valid_out <= 1'b0;
        end
