...
module mac16 (
    ...
    // VMA: Vector Merging Adder - using Kogge-Stone Adder (minimum depth)
    // Kogge-Stone has only 6 logic levels for 40-bit (vs ~8 for Han-Carlson)
    wire [ACC_WIDTH-1:0] vma_result;
    wire vma_cout;
    kogge_stone_adder #(.WIDTH(ACC_WIDTH)) u_vma (
        .a(mult_result_sum),
        .b(mult_result_carry),
        .sum(vma_result),
        .cout(vma_cout)
