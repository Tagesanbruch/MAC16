`timescale 1ns/1ps
//============================================================================
// 5-Stage Deep Pipeline Booth Multiplier (copied from exp_k)
// Target: 1154 MHz @ TT corner
//============================================================================
module mult16_booth_5stage (
    input  wire         clk,
    input  wire         rst_n,
    input  wire  [15:0] a,
    input  wire  [15:0] b,
    ...
    csa #(.WIDTH(40)) u_csa1 (.a(pp_aligned0), .b(pp_aligned1), .c(pp_aligned2), .sum(csa1_sum), .carry(csa1_carry));
    
    wire [39:0] csa2_sum, csa2_carry;
    csa #(.WIDTH(40)) u_csa2 (.a(pp_aligned3), .b(pp_aligned4), .c(pp_aligned5), .sum(csa2_sum), .carry(csa2_carry));
    ...
    csa #(.WIDTH(40)) u_csa3 (.a(row0_s2), .b(row1_s2), .c(row2_s2), .sum(csa3_sum), .carry(csa3_carry));
    
    wire [39:0] csa4_sum, csa4_carry;
    csa #(.WIDTH(40)) u_csa4 (.a(row3_s2), .b(row4_s2), .c(row5_s2), .sum(csa4_sum), .carry(csa4_carry));
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            row0_s3 <= 40'b0; row1_s3 <= 40'b0; row2_s3 <= 40'b0; row3_s3 <= 40'b0;
            valid_s3 <= 1'b0;
    ...
    //=========================================================================
    wire [39:0] final_product = final_sum_s4 + final_carry_s4;
