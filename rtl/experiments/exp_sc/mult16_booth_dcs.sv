`timescale 1ns/1ps
//============================================================================
// Experiment SC: DCS Booth Multiplier with 7:3 Counter (Stage4)
// 
...
// - Stage 4: 7:3 Counter + CSA (shifted) to 2 rows.
// - Stage 5: Output.
//
...
module mult16_booth_dcs (
    ...
    // Keep feedback as separate rows for 7:3 counter in Stage 4.
    // Pipeline register - Stage 3 (Register 7 rows, one is zero)
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            row4_s3 <= 40'b0; row5_s3 <= 40'b0; row6_s3 <= 40'b0;
            valid_s3 <= 1'b0;
    ...
    //=========================================================================
    // Stage 4: 7:3 Counter + CSA (shifted) (7→2)
    //=========================================================================
    wire [39:0] cnt_sum, cnt_carry1, cnt_carry2;
    counter_7to3_array #(.WIDTH(40)) u_cnt73 (
        .a(row0_s3),
        .b(row1_s3),
        .c(row2_s3),
        .d(row3_s3),
    ...
    // CSA: cnt_sum + (cnt_carry1<<1) + (cnt_carry2<<2) → final 2 vectors
    wire [39:0] csa6_sum, csa6_carry;
    csa #(.WIDTH(40)) u_csa6 (
        .a(cnt_sum),
        .b({cnt_carry1[38:0], 1'b0}),
        .c({cnt_carry2[37:0], 2'b0}),
        .sum(csa6_sum),
        .carry(csa6_carry)
    );
