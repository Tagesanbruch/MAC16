...
list_exp:
	...
	@echo ""
	@echo " Available PDKs:"
	@echo "  PDK=icsprout55      - L library "
	@echo "  PDK=icsprout55r     - R library (reduced)"
...
# VCD-based power analysis (more accurate than static estimation)
# Requires: tb_mac16.vcd in current directory (generated by verif target)
pwr_vcd:
	@if [ -z "$(LATEST_SYN_DIR)" ]; then \
		echo "[ERROR] No synthesis results found for EXP=$(EXP)."; \
		exit 1; \
	fi
	@if [ ! -f "tb_mac16.vcd" ]; then \
		echo "[ERROR] VCD file not found. Run 'make -f iEDA.mk EXP=$(EXP) verif' first."; \
		exit 1; \
	fi
	@echo "=============================================="
	@echo " VCD Power Analysis for Experiment: $(EXP)"
	@echo " VCD: tb_mac16.vcd"
	@echo "=============================================="
	../yosys-sta/bin/iEDA $(abspath scripts/power_analysis_vcd.tcl) \
		$(abspath ../yosys-sta/pdk/icsprout55) \
		"$(abspath $(LATEST_SYN_DIR))/$(DESIGN)-$(CLK_FREQ_MHZ)MHz/$(DESIGN).netlist.v" \
		$(abspath scripts/sdc/mac16.sdc) \
		"$(abspath tb_mac16.vcd)" \
		"tb_mac16/dut" \
		2>&1 | tee $(LATEST_SYN_DIR)/pwr_vcd.log
